<template>
  ...
  >
    ...
    h3.4V68.7z M37.7,61.9h-3.4v-3.4h3.4V61.9z M47.9,82.2h-3.4v-3.4h3.4V82.2z M47.9,75.4h-3.4V72h3.4V75.4z"
      ...
      ></path>
      <g v-else-if="iconType === 'jungle'" class="e-marker__icon e-marker__icon--jungle">
        <path
      ...
      <g class="e-marker__spinner" :class="{'e-marker__spinner--active': getPanningStatus}">
        <path
      ...
      <image
        ...
        :href="`${this.image}?fit=thumb&f=top_left&h=150&w=150&r=150`"
      ></image>
      <text opacity="0" class="e-marker__text" transform="matrix(1 0 0 1 0 13.9998)">
        <tspan x="50%" :y="[splitTitle.last ? '0' : '13']">{{ splitTitle.first }}</tspan>
        <tspan x="50%" y="13">{{ splitTitle.last }}</tspan>
      </text>
    </svg>
  </div>
</template>
  ...
  computed: {
    ...
    markerPoser() {
      ...
      Object.keys(childProps).forEach(key =>
        markerPose.addChild(
          this.$el.querySelector(`.e-marker__${key}`),
          childProps[key]
        )
      );
      return markerPose;
